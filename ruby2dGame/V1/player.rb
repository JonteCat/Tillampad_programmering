class Player
  ...
  def update()
    if Window.frames % 10 == 0
      ...
      #p new_y
      #
      next_block = get_block(new_x, new_y, nil)
      next_block_after = get_block(new_x + @move_x, new_y + @move_y, nil)
      ...
      if get_block(new_x, new_y, Button)
        puts "I am button"
      end

      if get_block(new_x, new_y, Block)
        puts "I am block"
      end
